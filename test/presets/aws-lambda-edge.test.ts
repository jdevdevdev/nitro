...
describe('nitro:preset:aws-lambda-edge', async () => {
  const ctx = await setupTest('@jdevdevdev/aws-lambda-edge-preset')
  ...
  testNitro(ctx, async () => {
    ...
    return async ({ url: rawRelativeUrl, headers, method, body }) => {
      ...
      const reqHeaders: CloudFrontHeaders = Object
        .fromEntries(
        Object
          .entries(headers || {})
          .map(([k, v]) => [
      ...
      for (const param of url.searchParams) {
        queryString = queryString
          ? [queryString, param.filter(Boolean).join("=")].join("&")
          : param.filter(Boolean).join("=");
      }
